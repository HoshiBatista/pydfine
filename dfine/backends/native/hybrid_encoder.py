"""HybridEncoder — native D-FINE port.

Ported from ``D-FINE/src/zoo/dfine/hybrid_encoder.py`` (Apache-2.0, © 2024 The
D-FINE Authors; itself modified from RT-DETR, © 2023 lyuwenyu). Changes from
upstream:

- Dropped the ``@register()`` decorator, ``__share__``, and the ``src.core`` import.
- Mutable default args replaced with ``None`` sentinels; ``super()`` modernised.
- Added :meth:`HybridEncoder.from_config`.

The AIFI transformer runs on the top pyramid level, then CCFM/GELAN fuses across
scales. Layer/parameter names match upstream so checkpoints load unchanged. Each
``ConvNormLayer_fuse``/``VGGBlock`` keeps its ``convert_to_deploy`` fusion for export.
"""
...
class HybridEncoder(nn.Module):
    """AIFI transformer on the top level + CCFM/GELAN cross-scale fusion.

    Consumes the backbone pyramid (``in_channels`` at ``feat_strides``) and emits a
    ``hidden_dim`` feature pyramid of the same spatial strides.
    """
    ...
    def __init__(
        ...
        super().__init__()
        in_channels = [512, 1024, 2048] if in_channels is None else in_channels
        feat_strides = [8, 16, 32] if feat_strides is None else feat_strides
        use_encoder_idx = [2] if use_encoder_idx is None else use_encoder_idx
    ...
    @classmethod
    def from_config(cls, cfg: DFINEConfig) -> HybridEncoder:
        """Build the encoder from a :class:`DFINEConfig`."""
        return cls(
            ...
            act=cfg.encoder_act,
            eval_spatial_size=[cfg.imgsz, cfg.imgsz],
        )
