"""DFINECriterion — native D-FINE port.

Ported from ``D-FINE/src/zoo/dfine/dfine_criterion.py`` (Apache-2.0, © 2024 The
D-FINE Authors; modified from RT-DETR, © 2023 lyuwenyu). Computes the full D-FINE
training loss over the decoder's training-mode outputs: VFL classification, L1 +
GIoU boxes, Fine-Grained Localization (FGL) and Decoupled Distillation Focal (DDF,
the GO-LSD term), summed over the final layer, auxiliary layers, encoder proposals
and the contrastive-denoising groups. Changes from upstream:

- Dropped ``@register()`` / ``__share__`` / ``__inject__`` and the ``src.core``
...
class DFINECriterion(nn.Module):
    ...
    @classmethod
    def from_config(cls, cfg: DFINEConfig) -> DFINECriterion:
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        """L1 + GIoU losses; target boxes are cxcywh normalized by image size."""
        assert "pred_boxes" in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs["pred_boxes"][idx]
        target_boxes = torch.cat([t["boxes"][i] for t, (_, i) in zip(targets, indices)], dim=0)
    ...
    def loss_local(self, outputs, targets, indices, num_boxes, T=5):
        """Fine-Grained Localization (FGL) + Decoupled Distillation Focal (DDF) losses."""
        losses = {}
    ...
    def forward(self, outputs, targets, **kwargs):
        """Compute the total loss dict over all supervised outputs."""
    ...
    def get_cdn_matched_indices(dn_meta, targets):
        """Build denoising-group prediction↔target indices."""
