"""DFINETransformer decoder — native D-FINE port.

Ported from ``D-FINE/src/zoo/dfine/dfine_decoder.py`` (Apache-2.0, © 2024 The
D-FINE Authors; modified from RT-DETR, © 2023 lyuwenyu). Deformable-attention DETR
decoder with Fine-grained Distribution Refinement (FDR), a Location Quality
Estimator (LQE), and contrastive denoising. Changes from upstream:

- Dropped ``@register()``, ``__share__``, and the ``src.core`` import; helpers now
...
class MSDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            reference_points: ``[bs, query_length, n_levels, 2 or 4]`` in ``[0, 1]``.
            value: preprocessed per-level values.
...
class Integral(nn.Module):
    """Compute a target location from a distribution: ``sum{Pr(n) * W(n)}``.

    ``Pr(n)`` is the softmax over the discrete bins; ``W(n)`` is the Weighting
    Function. Static (no learnable params).
    """
...
class TransformerDecoder(nn.Module):
    """FDR decoder: refines box distributions residually across layers."""
...
class DFINETransformer(nn.Module):
    """DETR-style deformable decoder with FDR + LQE + contrastive denoising."""
    ...
    def from_config(
        ...
        *,
        ...
        return cls(
            ...
            label_noise_ratio=cfg.label_noise_ratio,
            box_noise_scale=cfg.box_noise_scale,
            eval_spatial_size=[cfg.imgsz, cfg.imgsz],
            eval_idx=cfg.eval_idx,
            aux_loss=cfg.aux_loss,
            cross_attn_method=cfg.decoder_method,
            query_select_method=cfg.query_select_method,
            reg_max=cfg.reg_max,
